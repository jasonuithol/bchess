...
# uci.c uses pthreads for lazy SMP.
LIBRARIES="-lpthread"
...
echo $COMPILER $LANGUAGE $WARNINGS $TARGET_ARCH ${!OPTIMISE_VAR} $* $MAIN_FILE $SOURCES $LIBRARIES

$COMPILER $LANGUAGE $WARNINGS $TARGET_ARCH ${!OPTIMISE_VAR} $* $MAIN_FILE $SOURCES $LIBRARIES
